...
def preview_mode():
    """Run preview mode - NO EMAILS SENT"""
    workflow = st.session_state.workflow_manager

    st.info("🔍 **PREVIEW MODE** - No emails will be sent!")

    with st.spinner("🔍 Running preview mode analysis..."):
        # Run the actual preview mode (NO EMAILS SENT)
    ...
    if not employees_needing_alerts:
        ...
        st.warning(f"⚠️ {len(employees_needing_alerts)} employees would receive alerts (but NO emails sent in preview mode)")
    ...
    # Final confirmation
    st.success("🔍 **PREVIEW MODE COMPLETE** - No emails were sent! Check the console logs for detailed preview information.")

# Main page content
st.title("📊 Employee Hours Monitoring")
st.markdown("Monitor employee work hours - Optimized for accuracy")
...
with col1:
    if st.button("🔍 Preview Mode", use_container_width=True, type="primary",
                help="See who would receive alerts"):
        preview_mode()

with col2:
    if st.button("📧 Run Monitoring", use_container_width=True, type="secondary",
                help="Run monitoring workflow and send emails"):
