...
class EmailService:
    ...
    def send_low_hours_alert(self, real_employee_data: Dict, preview_mode: bool = False) -> bool:
        """
        Send email alert for low work hours using REAL employee data
        Uses your existing low_hours_email.html template

        Args:
            real_employee_data: Employee data dictionary
            preview_mode: If True, only shows preview without sending email
        """
        try:
            ...
            # PREVIEW MODE: Only show preview, don't send email
            if preview_mode:
                logger.info(f"🔍 PREVIEW MODE: Email preview for {real_employee_data['name']}")
                self._print_real_email_preview(real_employee_data, preview_mode=True)
                return True  # Return True to indicate preview was successful
    ...
    def _print_real_email_preview(self, real_employee_data: Dict, preview_mode: bool = False):
        """
        Print email preview when actual sending fails or in preview mode
        """
        shortfall_hours = real_employee_data.get('shortfall', 0)
        shortfall_minutes = int(shortfall_hours * 60)
        ...
        print("\n" + "="*70)
        if preview_mode:
            print("🔍 EMAIL PREVIEW MODE - NO EMAIL SENT")
        else:
