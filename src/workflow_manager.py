...
class WorkflowManager:
    ...
    def _process_employee_preview(self, employee: Dict, work_week_start: datetime, work_week_end: datetime) -> Dict:
        """Process individual employee for PREVIEW MODE - NO EMAILS SENT"""
        employee_id = employee.get('id')
        employee_email = employee.get('email')
        employee_name = employee.get('name', 'Employee')
        ...
        # Get leave days with force refresh for real-time accuracy
        leave_days = self._get_working_day_leaves_count_realtime(
            employee_name, work_week_start, work_week_end
        )
        ...
        if leave_days >= 5:
            logger.info(f"🏖️ {employee_name} on full leave ({leave_days} days) - NO EMAIL WOULD BE SENT")
            return {'status': 'on_full_leave', 'employee': employee_name, 'leave_days': leave_days}

        if actual_hours_worked >= acceptable_hours:
            logger.debug(f"✅ {employee_name}: {actual_hours_worked:.1f}h >= {acceptable_hours:.1f}h - NO EMAIL NEEDED")
            return {'status': 'hours_met', 'employee': employee_name, 'hours': actual_hours_worked}

        # Alert would be needed (but NOT sent in preview mode)
        shortfall = required_hours - actual_hours_worked

        logger.info(f"⚠️ PREVIEW: Alert WOULD BE SENT to {employee_name}:")
        logger.info(f"  Hours: {actual_hours_worked:.1f}h < {acceptable_hours:.1f}h")
        logger.info(f"  Required: {required_hours:.1f}h, Shortfall: {shortfall:.1f}h")
        logger.info(f"  Leave days: {leave_days}")
        logger.info(f"  📧 EMAIL WOULD GO TO: {employee_email}")

        # Create email data for preview with idle time information
        email_data = {
            'email': employee_email,
            'name': employee_name,
            'week_start': work_week_start.strftime('%Y-%m-%d'),
        ...
        # Show email preview (but don't send)
        self.email_service.send_low_hours_alert(email_data, preview_mode=True)

        return {'status': 'would_send_alert', 'employee': employee_name, 'shortfall': shortfall, 'email': employee_email}
    ...
    def run_preview_mode(self):
        """Run preview mode - analyze employees but DO NOT send emails"""
        start_time = datetime.now()
        logger.info("="*60)
        ...
        if not employees:
            logger.error("No employees found. Preview cannot proceed.")
            return
        ...
        # Process employees for preview (no emails sent)
        for employee in employees:
            try:
                processed_count += 1
                employee_name = employee.get('name', 'Unknown')
                ...
                # Process employee for preview only
                result = self._process_employee_preview(employee, work_week_start, work_week_end)

                if result['status'] == 'would_send_alert':
                    alerts_would_be_sent += 1
                elif result['status'] == 'on_full_leave':
                    employees_on_leave += 1
                elif result['status'] == 'hours_met':
                    employees_meeting_hours += 1
        ...
        logger.info(f"Total employees processed: {processed_count}")
        logger.info(f"Employees excluded: {excluded_count}")
        logger.info(f"Employees on full leave: {employees_on_leave}")
        logger.info(f"Employees meeting hours: {employees_meeting_hours}")
        logger.info(f"⚠️ ALERTS THAT WOULD BE SENT: {alerts_would_be_sent}")
        logger.info(f"Errors: {errors_count}")
        logger.info(f"Execution time: {execution_time:.2f} seconds")
        logger.info("🔍 NO EMAILS WERE SENT - THIS WAS PREVIEW MODE")
        logger.info("="*60)
